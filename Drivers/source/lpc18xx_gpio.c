 ...
 * @param[in]	portNum	Port number value, should be in range from 0 to 4
 * @param[in]	bitValue Value that contains all bits on GPIO to toggle, should
 * 				be in range from 0 to 0xFFFFFFFF.
 * 				example: value 0x5 to toggle bit 0 and bit 1.
 * @return		None
 *
 * Note:
 * - For all bits that has been set as input direction, this function will
 ...
 * will not be effected by this function.
 **********************************************************************/
void GPIO_ToggleValue(uint8_t portNum, uint32_t bitValue)
{
	LPC_GPIO_PORT->NOT[portNum] = bitValue;
}
